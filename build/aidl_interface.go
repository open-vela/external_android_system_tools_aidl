...
// AidlNotFrozenInfo keeps the list of not-frozen version which shouldn't be used by other modules.
type AidlNotFrozenInfo struct {
	notFrozen []string
}

var AidlNotFrozenInfoProvider = blueprint.NewMutatorProvider(AidlNotFrozenInfo{}, "checkUnstableModule")

// check if an unstable module is in use
func checkUnstableModuleMutator(mctx android.BottomUpMutatorContext) {
	...
	if gen, ok := mctx.Module().(*aidlGenRule); ok {
		if gen.properties.NotFrozen {
			mctx.SetProvider(AidlNotFrozenInfoProvider, AidlNotFrozenInfo{
	...
	mctx.VisitDirectDeps(func(dep android.Module) {
		info := mctx.OtherModuleProvider(dep, AidlNotFrozenInfoProvider).(AidlNotFrozenInfo)
		notFrozen = append(notFrozen, info.notFrozen...)
	})
	if len(notFrozen) > 0 {
		// Ignore when a lib module is generated by aidl_interface, but propagate "notFrozen" to parents
		if isAidlGeneratedModule(mctx.Module()) {
			mctx.SetProvider(AidlNotFrozenInfoProvider, AidlNotFrozenInfo{
				notFrozen: notFrozen,
			})
			return
		...
		for _, name := range notFrozen {
			mctx.ModuleErrorf("%v is disallowed in release version because it is unstable, and its \"owner\" property is missing.",
				name)
		}
	}
}
